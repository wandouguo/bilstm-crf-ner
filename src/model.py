...
class Model(object):
    ...
    def fit(self, batch_data, session):
        ...
        self.optimise = tf.train.GradientDescentOptimizer(learning_rate=0.01).minimize(self.crf_loss)
        loss, _ = session.run([self.crf_loss, self.optimise], feed_dict=feed_dict)
        return loss
...
if __name__ == '__main__':
    ...
    with tf.Session() as session:
        ...
        for i in range(0, 200):
            session.run(tf.local_variables_initializer())
            batch_data = {"data": [[0, 1, 2], [1, 2, 3]],
                          "tags": [[0, 1, 2], [0, 1, 2]],
                          "seg_length": [2, 2]
                          }

            print(model.fit(batch_data, session))
